...
# Подключение библиотеки для работы с интервальными числами
...
def plot_tol(axis, A, b):
    ...
    axis.plot_surface(x_1_, x_2_, list_tol, cmap='Blues')
    # Отмечаем на графике точку (sol[0], sol[1], max_val)
    axis.scatter(sol[0], sol[1], max_val, color='red', s=70)
...
def plot_tol_functional(axis, A, b):
    ...
    axis.contourf(xx, yy, zz, levels=1, colors=['darkblue', 'lightcoral'])
...
# Создание фигур для визуализации
...
for index, (A, b) in enumerate(systems):
   ...
   is_tolerance_set_empty(A, b_corrected)
   ...
   is_tolerance_set_empty(A_corrected, b)
...
# Обновление размещения элементов
...
# Сохранение всех графиков в папку
